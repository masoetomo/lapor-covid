<template>
  <app-layout>
    <template #header>
      <header-master-balai></header-master-balai>
    </template>
    <template #main>
      <div class="py-12">
                ...
                <div class="">
                  <div class="mb-4">
                    ...
                    <input
                      ...
                      v-model="form.tipeBalai"
                    />
                    <div v-if="errors.tipeBalai != null" class="text-red-500">
                      {{ errors.tipeBalai }}
                    </div>
                  </div>
                  <div class="mb-4">
...
import HeaderMasterBalai from '../Layouts/HeaderMasterBalai.vue'
...
export default {
    ...
    methods: {
        ...
        async getMasterData(){
            ...
            console.log(pageNum);
            try {
            const res = await axios.get('api/databalai?page='+pageNum);
        ...
        try {
          const res = await axios.delete('api/databalai/'+data.id);
          if(res.status === 200){
            Toast.fire({
              icon:'success',
              title: res.data
      ...
      async updateDataMaster(data){
        try {
          const res = await axios.put('api/databalai/'+data.id, data);
          if(res.status === 200){
            Toast.fire({
              icon:'success',
              title: res.data
      ...
      async showDataMaster(id){
        try {
          const res = await axios.get('api/databalai/'+id);
          // console.log(res.data.tanggal);
          this.form.tipeBalai = res.data.tipe_balai;
