<template>
  <app-layout>
    <template #header>
      <header-master-loka></header-master-loka>
    </template>
    <template #main>
      <div class="py-12">
...
import HeaderMasterLoka from '../Layouts/HeaderMasterLoka.vue'
...
export default {
    ...
    methods: {
        ...
        async getMasterData(){
            ...
            console.log(pageNum);
            try {
            const res = await axios.get('api/dataloka?page='+pageNum);
        ...
        try {
          const res = await axios.delete('api/dataloka/'+data.id);
          if(res.status === 200){
            Toast.fire({
              icon:'success',
              title: res.data
      ...
      async updateDataMaster(data){
        try {
          const res = await axios.put('api/dataloka/'+data.id, data);
          if(res.status === 200){
            Toast.fire({
              icon:'success',
              title: res.data
      ...
      async showDataMaster(id){
        try {
          const res = await axios.get('api/dataloka/'+id);
